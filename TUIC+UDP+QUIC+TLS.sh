...
# 获取真实IP
REAL_IP=$(curl -s https://api.ipify.org || curl -s https://ipinfo.io/ip || echo "unknown")
if [[ "$REAL_IP" == "unknown" ]]; then
    REAL_IP=$(ip route get 1 | awk '{print $NF; exit}')
fi

ENCODE=$(echo -n "${UUID}:${PSK}" | base64 -w 0)
LINK="tuic://${ENCODE}@${REAL_IP}:${PORT}?alpn=h3&congestion_control=bbr&sni=${SERVER_NAME}&udp_relay_mode=native&allow_insecure=1#tuic_node"

echo -e "\n${GREEN}✅ TUIC 节点部署完成${NC}"
echo -e "${GREEN}外网 IP     :${NC} $REAL_IP"
...
# 上传配置到JSONBin
upload_to_jsonbin "$REAL_IP" "$LINK" "$(cat "$V2RAYN_CFG")" "$SPEED_RESULT"
